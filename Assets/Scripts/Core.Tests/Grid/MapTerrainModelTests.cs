...
namespace ProjectAstra.Core.Tests.Grid
{
    ...
    public class MapTerrainModelTests
    {
        ...
        public void SetUp()
        {
            ...
            so.FindProperty("height").intValue = 2;
        ...
        [TearDown]
        public void TearDown() => Object.DestroyImmediate(map);

        [Test]
        public void TerrainAt_ReturnsPaintedTerrain()
        {
            Assert.AreEqual(TerrainType.Plain, map.TerrainAt(0, 0));
            Assert.AreEqual(TerrainType.Road, map.TerrainAt(2, 0));
            Assert.AreEqual(TerrainType.Water, map.TerrainAt(0, 1));
            Assert.AreEqual(TerrainType.Fort, map.TerrainAt(2, 1));
        }
        ...
        public void TerrainAt_UsesRowMajorIndexing()
        {
            ...
            Assert.AreEqual(TerrainType.Wall, map.TerrainAt(1, 1));
        }

        [Test]
        public void TerrainAt_OutOfBounds_ReturnsVoid()
        {
            Assert.AreEqual(TerrainType.Void, map.TerrainAt(-1, 0));
            Assert.AreEqual(TerrainType.Void, map.TerrainAt(3, 0));
            Assert.AreEqual(TerrainType.Void, map.TerrainAt(0, 2));
        }

        [Test]
        public void MapId_RoundTrips()
        {
            Assert.AreEqual("test_map", map.MapId);
        }
        ...
        public void MapCatalog_GetByStringId_ResolvesMap()
        {
            ...
            var mapsProp = cso.FindProperty("maps");
            mapsProp.arraySize = 1;
            mapsProp.GetArrayElementAtIndex(0).objectReferenceValue = map;
            cso.ApplyModifiedPropertiesWithoutUndo();

            Assert.AreSame(map, catalog.Get("test_map"));

...
namespace ProjectAstra.Core.Tests.Gurukul
{
    ...
    public class EdgeIndicatorSolverTests
    {
        ...
        public void SomethingDueEast_SitsOnTheRightEdge()
        {
            ...
            Assert.AreEqual(GurukulScreenSpace.CanvasWidth - Inset, canvas.x, 0.5f);
            Assert.AreEqual(GurukulScreenSpace.CanvasHeight * 0.5f, canvas.y, 0.5f);
        }
        ...
        public void SomethingDueNorth_SitsOnTheTopEdge()
        {
            ...
            Assert.AreEqual(GurukulScreenSpace.CanvasWidth * 0.5f, canvas.x, 0.5f);
            Assert.AreEqual(GurukulScreenSpace.CanvasHeight - Inset, canvas.y, 0.5f);
        }
        ...
        public void TheIndicatorAlwaysStaysInsideTheCanvas()
        {
            ...
            foreach (Vector2 offset in directions)
            {
                ...
                Assert.LessOrEqual(canvas.x, GurukulScreenSpace.CanvasWidth - Inset + 0.5f);
                Assert.GreaterOrEqual(canvas.y, Inset - 0.5f);
                Assert.LessOrEqual(canvas.y, GurukulScreenSpace.CanvasHeight - Inset + 0.5f);
            }
        }
        ...
        {
            Assert.AreEqual(4f, GurukulScreenSpace.CanvasScale, 0.0001f);
            Assert.AreEqual(240f, GurukulScreenSpace.ToCanvas(60f), 0.0001f);
        }

        [Test]
        public void WorldToCanvas_PutsTheCameraCentreInTheMiddle()
        {
            Vector2 canvas = GurukulScreenSpace.WorldToCanvas(Camera, Camera, View);

            Assert.AreEqual(GurukulScreenSpace.CanvasWidth * 0.5f, canvas.x, 0.001f);
            Assert.AreEqual(GurukulScreenSpace.CanvasHeight * 0.5f, canvas.y, 0.001f);
        }
    }
}

...
{
    [TestFixture]
    public class GurukulMoverTests
    {
        ...
        [SetUp]
        public void SetUp() => map = new GurukulCollisionMap(16, 16);   // 8x8 tiles

        private Vector2 Move(Vector2 from, Facing? direction, float deltaTime, out bool moved) =>
            GurukulMover.Move(map, from, Footprint, direction, deltaTime, out moved);
        ...
        public void OpenGround_MovesAtTheAuthoredSpeed()
        {
            ...
            Assert.IsTrue(moved);
            Assert.AreEqual(2f + GurukulMover.DefaultSpeedTilesPerSecond, result.x, 0.001f);
        ...
        {
            const float dt = 0.1f;
            float distance = GurukulMover.DefaultSpeedTilesPerSecond * dt;
        ...
        {
            Rect box = GurukulMover.FootprintAt(new Vector2(3f, 5f), Footprint);

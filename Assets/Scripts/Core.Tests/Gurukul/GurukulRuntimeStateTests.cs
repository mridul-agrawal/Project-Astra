...
namespace ProjectAstra.Core.Tests.Gurukul
{
    ...
    [TestFixture]
    public class GurukulRuntimeStateTests
    {
        private GurukulRuntimeState state;

        [SetUp]
        public void SetUp() => state = new GurukulRuntimeState("hub1");
        ...
        public void AnUnmentionedInteractable_FallsBackToItsAuthoredState()
        {
            Assert.AreEqual(GurukulInteractableState.Available,
                state.GetInteractableState("blackboard", GurukulInteractableState.Available));

            state.SetInteractableState("blackboard", GurukulInteractableState.Exhausted);
            Assert.AreEqual(GurukulInteractableState.Exhausted,
                state.GetInteractableState("blackboard", GurukulInteractableState.Available));
        }
        ...
        {
            state.SetInteractableState("door", GurukulInteractableState.Gated);
            state.SetInteractableState("door", GurukulInteractableState.Available);

            Assert.AreEqual(GurukulInteractableState.Available,
                state.GetInteractableState("door", GurukulInteractableState.Inactive));
        }
    }
}

...
{
    public struct DataHubProblem
    {
        public readonly string Message;
        public readonly UnityEngine.Object Asset;
        public DataHubProblem(UnityEngine.Object asset, string message) { Asset = asset; Message = message; }
    }
    ...
    {
        public static List<DataHubProblem> Collect()
        {
            var problems = new List<DataHubProblem>();
            ...
            foreach (UnitDefinition u in DataHubAssets.LoadAll<UnitDefinition>())
            {
                if (string.IsNullOrWhiteSpace(u.UnitId))
                    problems.Add(new DataHubProblem(u, $"{u.name}: empty unitId"));
                else if (idOwners.TryGetValue(u.UnitId, out UnitDefinition other))
                    problems.Add(new DataHubProblem(u, $"{u.name}: duplicate unitId '{u.UnitId}' (shared with {other.name})"));
                else
                    idOwners[u.UnitId] = u;

                if (db != null && !DataHubAssets.IsRegistered(db, u))
                    problems.Add(new DataHubProblem(u, $"{u.name}: not registered in UnitDatabase"));
                if (u.DefaultClass == null)
                    problems.Add(new DataHubProblem(u, $"{u.name}: no defaultClass assigned"));
                if (IsAllZero(u.BaseStats))
                    problems.Add(new DataHubProblem(u, $"{u.name}: base stats are all zero"));
            }

            foreach (ClassDefinition c in DataHubAssets.LoadAll<ClassDefinition>())
                if (IsAllZero(c.StatCaps))
                    problems.Add(new DataHubProblem(c, $"{c.name}: stat caps are all zero"));

            foreach (WeaponDefinition w in DataHubAssets.LoadAll<WeaponDefinition>())
            {
                var rt = w.ToRuntime();
                if (rt.minRange > rt.maxRange)
                    problems.Add(new DataHubProblem(w, $"{w.name}: minRange ({rt.minRange}) > maxRange ({rt.maxRange})"));
                if (rt.characterLocked && !string.IsNullOrEmpty(rt.ownerUnitId) && !idOwners.ContainsKey(rt.ownerUnitId))
                    problems.Add(new DataHubProblem(w, $"{w.name}: character-locked to unknown unitId '{rt.ownerUnitId}'"));
            }
            ...
            foreach (InventoryLoadout l in DataHubAssets.LoadAll<InventoryLoadout>())
            {
                ...
                for (int i = 0; i < items.Length; i++)
                    if (items[i] == null)
                        problems.Add(new DataHubProblem(l, $"{l.name}: item slot {i} is empty/missing"));
            }

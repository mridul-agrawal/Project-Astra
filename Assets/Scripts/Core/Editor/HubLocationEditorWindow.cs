...
namespace ProjectAstra.Core.Editor
{
    ...
    public class HubLocationEditorWindow : EditorWindow
    {
        ...
        private const int CellsPerTile = HubLocationData.CellsPerTile;
        private const string ArtFolder = "Assets/Gurukul/Art";

        private HubLocationData target;
        ...
        private readonly List<HubPropFootprint> props = new();
        private bool dirty;

        private HubLocationDatabase database;
        ...
        [MenuItem("Project Astra/Hub/Location Editor")]
        public static void Open()
        {
            var window = GetWindow<HubLocationEditorWindow>("Location Editor");
        ...
        private void OnGUI()
        {
            ...
            if (target == null)
            {
                EditorGUILayout.HelpBox("Pick a HubLocationData to edit, or create one from the asset menu.", MessageType.Info);
                return;
            }
        ...
        private void DrawTargetField()
        {
            ...
            var picked = (HubLocationData)EditorGUILayout.ObjectField("Location", target, typeof(HubLocationData), false);
        ...
        private void DrawPropOutlines(Rect rect)
        {
            ...
            foreach (HubPropFootprint prop in props)
        ...
        private void AddPropFromDrag(Vector2Int from, Vector2Int to)
        {
            ...
            props.Add(new HubPropFootprint
            {
                propId = string.IsNullOrEmpty(newPropId) ? $"prop_{props.Count}" : newPropId,
                startsSolid = true,
                footprint = new Rect(
                    minX * HubCollisionMap.CellSize, minY * HubCollisionMap.CellSize,
                    (maxX - minX + 1) * HubCollisionMap.CellSize,
                    (maxY - minY + 1) * HubCollisionMap.CellSize)
            });
            dirty = true;
        }
        ...
        {
            float cell = HubCollisionMap.CellSize;
        ...
        private void LoadTarget(HubLocationData picked)
        {
            target = picked;
            dirty = false;
        ...
        {
            string[] guids = AssetDatabase.FindAssets("t:HubLocationDatabase");
            database = guids.Length > 0
                ? AssetDatabase.LoadAssetAtPath<HubLocationDatabase>(AssetDatabase.GUIDToAssetPath(guids[0]))
                : null;

...
namespace ProjectAstra.Core.Gurukul
{
    ...
    public sealed class GurukulMarkerManager : MonoBehaviour
    {
        ...
        private void LateUpdate()
        {
            ...
            SyncMarkers(GurukulProgressService.Instance.Objectives, active);
            edgeIndicators?.Render(offScreen);
        }
        ...
        {
            GurukulActor actor = GurukulWorld.FindActor(targetId);
            if (actor != null) return actor.transform;

            GurukulInteractable interactable = GurukulWorld.FindInteractable(targetId);

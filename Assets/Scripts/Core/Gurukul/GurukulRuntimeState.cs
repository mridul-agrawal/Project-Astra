...
{
    [Serializable]
    public struct GurukulInteractableRecord
    {
    ...
    [Serializable]
    public struct GurukulRelocationRecord
    {
    ...
    [Serializable]
    public struct GurukulConversationRecord
    {
        public string characterId;
    ...
    public class GurukulRuntimeState : IConversationMemory
    {
        ...
        [SerializeField] private List<GurukulInteractableRecord> interactables = new();
        [SerializeField] private List<GurukulRelocationRecord> relocations = new();
        [SerializeField] private List<GurukulConversationRecord> conversationOverrides = new();
        ...
        public IReadOnlyList<GurukulRelocationRecord> Relocations => relocations;
        public bool Departed => departed;

        public GurukulRuntimeState(string visitId)
        {
        ...
        public GurukulInteractableState GetInteractableState(string interactableId, GurukulInteractableState fallback)
        {
        ...
        public void SetInteractableState(string interactableId, GurukulInteractableState state)
        {
            if (string.IsNullOrEmpty(interactableId)) return;
            for (int i = 0; i < interactables.Count; i++)
            {
                if (interactables[i].interactableId != interactableId) continue;
                interactables[i] = new GurukulInteractableRecord { interactableId = interactableId, state = state };
                return;
            }
            interactables.Add(new GurukulInteractableRecord { interactableId = interactableId, state = state });
        }

        public bool TryGetRelocation(string characterId, out GurukulRelocationRecord record)
        {
        ...
        public void Relocate(string characterId, string locationId, Vector2 position, Facing facing)
        {
            ...
            var record = new GurukulRelocationRecord
            {
        ...
        {
            if (string.IsNullOrEmpty(characterId)) return;
            var record = new GurukulConversationRecord { characterId = characterId, conversationId = conversationId };

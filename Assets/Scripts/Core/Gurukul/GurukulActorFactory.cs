...
namespace ProjectAstra.Core.Gurukul
{
    ...
    // battle.
    public static class GurukulActorFactory
    {
        private const string SpriteChildName = "Sprite";

        public static GurukulActor Create(UnitDefinition definition, Transform parent, Vector2 position,
            Facing facing, string conversationId = null, bool solid = true)
        {
            if (definition == null)
            {
                Debug.LogError("[GurukulActorFactory] Cannot build an actor from a missing UnitDefinition.");
                return null;
            }
            ...
            var actor = root.AddComponent<GurukulActor>();

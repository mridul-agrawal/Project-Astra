...
namespace ProjectAstra.Core.Gurukul
{
    ...
    public sealed class GurukulActor : MonoBehaviour
    {
        ...
        public Rect Footprint => GurukulMover.FootprintAt(Position, footprintOffset);
        ...
        {
            if (GurukulLocationService.Instance == null) return;
            stamped = Footprint;
            GurukulLocationService.Instance.Collision.Stamp(stamped);
        }

        private void UnstampFootprint()
        {
            if (GurukulLocationService.Instance == null || stamped.width <= 0f) return;
            GurukulLocationService.Instance.Collision.Unstamp(stamped);
            stamped = default;
        }
    }
}

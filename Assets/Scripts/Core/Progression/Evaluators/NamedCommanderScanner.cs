...
{
    // On Start, walks the scene's TestUnits. If any enemy unit's
    // UnitDefinition has IsNamedCommander = true, raises the
    // WarLedgerServices flag so the Ledger-trigger predicate fires even when
    // the named commander survives.
    //
    ...
    public class NamedCommanderScanner : MonoBehaviour
    {
        private void Start()
        {
            ...
            foreach (var u in units)
            {
                ...
                // Read the serialized UnitDefinition directly — Start
                // execution order isn't guaranteed relative to TestUnit.Start
                // which binds UnitInstance.

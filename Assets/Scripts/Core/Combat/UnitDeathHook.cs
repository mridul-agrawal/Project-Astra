...
namespace ProjectAstra.Core.Combat
{
    ...
    // (and from future damage sources — poison, environmental). Performs:
    ...
    //   5. Disable the GameObject so it disappears from the map (Lords stay
    //      visible so LordDeathWatcher can fade them).
    public static class UnitDeathHook
    {
        public static void HandleDeath(TestUnit victim, TestUnit killer, UnitDeathEventChannel channel)
        {
            ...
            // Unregister BEFORE raising so listeners (BattleVictoryWatcher) see
            // the post-death faction counts. Otherwise "all enemies dead" can
            // never trigger, because the last-to-die victim still appears
            ...
            // Lord fade + dialogue + GameOver is driven by LordDeathWatcher;
            // keep the sprite visible so its coroutine has something to fade.
            if (!lordDeath)
